...
def load_config() -> Config:
    ...
    reels = _int_env("REELS_PER_WEEK", 5)
    reels = reels if 1 <= reels <= 40 else 5
...
def init_db() -> None:
    with _conn() as c:
        ...
        for col in ("reference_url", "thumbnail_url", "hashtags"):
            if col not in cols:
...
def add_reel(plan_id: int, idx: int, **fields) -> int:
    with _conn() as c:
        cur = c.execute(
            "INSERT INTO reels (plan_id, idx, day_label, concept, inspiration, reference_url, "
            "thumbnail_url, hook, caption, hashtags, shoot, audio, status, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'todo', ?)",
...
async def scrape_handles(handles: Sequence[str], results_per: int = 6) -> list[dict]:
    ...
    for it in items:
        if not isinstance(it, dict):
            continue
        music = ""
        mi = it.get("musicInfo") or it.get("musicCanonicalId") or {}
...
def rank_references(posts: list[dict], limit: int = 20) -> list[dict]:
    """Top scraped posts that can serve as real references (must have a URL).

    Videos/reels are preferred (they carry view counts), then everything else,
    ranked by engagement. Returns at most `limit`.
    """
    usable = [p for p in posts if p.get("url")]
    videos = [p for p in usable if (p.get("views") or 0) > 0 or "video" in str(p.get("type", "")).lower()]
    others = [p for p in usable if p not in videos]
    ranked = sorted(videos, key=_ref_score, reverse=True) + sorted(others, key=_ref_score, reverse=True)
    return ranked[:limit]
...
# =========================================================================== #
_GEN_SYSTEM = (
    "You are a senior short-form video strategist for a creator agency. You turn a set of "
    "REAL, currently-performing reference videos into a tight, directive shoot brief the "
    "creator's team can execute today. You NEVER invent references: every idea is built on "
    "one of the real reference videos provided to you, cited by its number. The brief fields "
    "(Concept, Shot, Audio, Hook, Hashtags) are professional and concise — an instruction "
    "sheet for the team, one line each, no filler, no restating the concept. The Caption is "
    "different: write it in the creator's own casual, lowercase, flirty posting voice, "
    "paste-ready. Keep the two registers separate. Stay strictly within her niche and tone."
)


def _build_gen_prompt(program: sqlite3.Row, model_name: str, refs_text: str, n: int) -> str:
    comps = program_competitors(program)
    parts = [
        f"Build a {n}-idea content plan for {model_name}.",
        "",
    ...
    if refs_text:
        parts += [
            "REAL reference videos currently performing in her niche (each numbered). Build "
            "each idea on the SINGLE most relevant one and cite it with its number in "
            "'reference_index'. Use ONLY these numbers — never invent a video:",
            "",
            refs_text,
            "",
        ]
        ...
        parts += [
            "No live reference videos are available right now. Set reference_index to -1 for "
            "every idea (the reference will read 'no live reference found'). Still produce "
            "strong, on-niche ideas.",
            "",
        ]
    parts += [
        f"Produce exactly {n} ideas. For EACH idea give:",
        "- reference_index: the number of the reference video this idea is based on (or -1).",
        "- concept: one line — what the video is. Don't restate it in other fields.",
        "- shot: how to film it — framing, action, expression. Specific and short.",
        "- audio: the trending sound by name (use the reference's audio if one is given).",
        "- hook: the on-screen text line (a few scroll-stopping words).",
        "- caption: paste-ready, in her casual lowercase flirty voice.",
        "- hashtags: one line of 4–8 relevant hashtags.",
        "Keep the brief fields professional and tight; keep the caption in her voice.",
    ]
    return "\n".join(p for p in parts if p is not None)


def _generate_plan_sync(program_data: dict, model_name: str, refs_text: str, n: int) -> list[dict]:
    """Blocking Anthropic call. Run via asyncio.to_thread. Returns a list of idea dicts."""
    import anthropic
    ...
    prompt = _build_gen_prompt(_P(prog_obj), model_name, refs_text, n)
    ...
    return [idea.model_dump() for idea in resp.parsed_output.ideas]


async def generate_plan(model_id: int, created_by: int) -> Optional[int]:
    """Scrape real references -> Claude -> persist a plan. Returns plan_id or None."""
    program = get_program(model_id)
    model = get_user(model_id)
    if program is None or model is None:
        return None
    n = program["reels_per_week"] or (CONFIG.reels_per_week if CONFIG else 5)
    ...
    ideas = await asyncio.to_thread(
        _generate_plan_sync, program_data, model["display_name"], refs_text, n)

    week = _week_start(datetime.now(_tz()).date())
    plan_id = create_plan(model_id, week, "", created_by)
    for i, idea in enumerate(ideas, start=1):
        ridx = idea.get("reference_index", -1)
        ref = refs[ridx - 1] if isinstance(ridx, int) and 1 <= ridx <= len(refs) else None
        add_reel(
            plan_id, i,
            ...
            reference_url=(ref["url"] if ref else "no live reference found"),
            thumbnail_url=(ref.get("thumbnail") if ref else None) or None,
        )
    return plan_id
...
def _idea_blocks(r: sqlite3.Row, include_images: bool = True) -> list[dict]:
    blocks: list[dict] = []
    thumb = r["thumbnail_url"] if "thumbnail_url" in r.keys() else None
    if include_images and thumb:
        blocks.append(_image(thumb))                       # visual break between ideas
    blocks.append(_heading(f"Idea {r['idx']}", 3))
    blocks.append(_field("Concept", r["concept"]))
    ...
    blocks.append(_field("Shot", r["shoot"]))
...
def _plan_blocks(plan_id: int, include_images: bool = True) -> list[dict]:
    blocks: list[dict] = []
    for r in list_reels(plan_id):
        blocks += _idea_blocks(r, include_images=include_images)
    return blocks
...
async def notion_create_plan_page(plan_id: int):
    """Build the content-plan page per the output spec. Returns (page_id, url)."""
    plan = get_plan(plan_id)
    if plan is None:
        return None, None
    model = get_user(plan["model_id"])
    ...
    blocks = _plan_blocks(plan_id, include_images=True)
    try:
        data = await _notion_post("/pages", _payload(blocks))
    except Exception as exc:  # noqa: BLE001 — usually an expiring thumbnail; retry text-only
        log.warning("Notion page w/ images failed (%s); retrying without images", exc)
        blocks = _plan_blocks(plan_id, include_images=False)
        data = await _notion_post("/pages", _payload(blocks))
