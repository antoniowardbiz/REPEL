...
def load_config() -> Config:
    ...
    try:
        tz_offset = int(os.getenv("TZ_OFFSET", "0").strip() or "0")
    except ValueError:
        tz_offset = 0
    return Config(
        bot_token=token,
...
def list_requests_for_reminders() -> list:
    with _conn() as c:
        return list(c.execute(
            "SELECT * FROM requests WHERE status = ? AND eta_at IS NOT NULL",
            (RequestStatus.ACCEPTED.value,)).fetchall())
...
MA_ETA_DAY, MA_ETA_TIME, MA_REASON, MA_UPLOAD = range(4)
...
# ETA time choices shown to the model (label, HHMM).
ETA_TIMES = [("9am", "0900"), ("12pm", "1200"), ("3pm", "1500"), ("6pm", "1800"), ("9pm", "2100")]
...
def _eta_day_picker() -> InlineKeyboardMarkup:
    today = datetime.now(_tz()).date()
    buttons = []
    for i in range(7):
        d = today + timedelta(days=i)
        if i == 0:
            label = f"Today {d.strftime('%a %d')}"
        elif i == 1:
            label = f"Tmrw {d.strftime('%a %d')}"
        else:
            label = d.strftime("%a %d %b")
        buttons.append(InlineKeyboardButton(label, callback_data=f"eta:day:{d.isoformat()}"))
    rows = _rows(buttons, per_row=2)
    rows.append([InlineKeyboardButton("✖ Cancel", callback_data="eta:cancel")])
    return InlineKeyboardMarkup(rows)


def _eta_time_picker() -> InlineKeyboardMarkup:
    buttons = [InlineKeyboardButton(lbl, callback_data=f"eta:time:{val}") for lbl, val in ETA_TIMES]
    rows = _rows(buttons, per_row=3)
    rows.append([InlineKeyboardButton("✖ Cancel", callback_data="eta:cancel")])
    return InlineKeyboardMarkup(rows)


def _format_eta(local_dt: datetime) -> str:
    hour12 = local_dt.strftime("%I").lstrip("0") or "12"
    return (f"{local_dt.strftime('%a %d %b')}, {hour12}:{local_dt.strftime('%M')}"
            f"{local_dt.strftime('%p').lower()}")
...
async def req_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send(update, context, "Great! Which day will it be ready?", reply_markup=_eta_day_picker())
    return MA_ETA_DAY


async def ma_eta_day(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    ma = context.user_data.get("ma") or {}
    req = get_request(ma.get("id")) if ma.get("id") else None
    if not _owns_request(update, req) or req["status"] != RequestStatus.PENDING.value:
        context.user_data.pop("ma", None)
        await q.edit_message_text("That request is no longer pending.")
        return ConversationHandler.END
    ma["eta_day"] = q.data.split(":")[2]   # YYYY-MM-DD
    context.user_data["ma"] = ma
    await q.edit_message_text("What time?", reply_markup=_eta_time_picker())
    return MA_ETA_TIME


async def ma_eta_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    ma = context.user_data.get("ma") or {}
    req_id = ma.get("id")
    ...
    if not _owns_request(update, req) or not day or req["status"] != RequestStatus.PENDING.value:
        context.user_data.pop("ma", None)
        await q.edit_message_text("That request is no longer pending.")
        return ConversationHandler.END
    hhmm = q.data.split(":")[2]
    d = date.fromisoformat(day)
    local_dt = datetime(d.year, d.month, d.day, int(hhmm[:2]), int(hhmm[2:]), tzinfo=_tz())
    eta_at = local_dt.astimezone(timezone.utc)
    eta_str = _format_eta(local_dt)
    update_request(req_id, status=RequestStatus.ACCEPTED.value, eta=eta_str,
                   eta_at=eta_at.isoformat(), reminded_24h=0, reminded_overdue=0)
    req = get_request(req_id)
...
async def ma_eta_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    context.user_data.pop("ma", None)
    await q.edit_message_text(
        "Cancelled — the request is still pending; accept it again from /queue when ready.")
...
def model_action_conversation() -> ConversationHandler:
    return ConversationHandler(
        ...
        states={
            MA_ETA_DAY: [
                CallbackQueryHandler(ma_eta_day, pattern=r"^eta:day:"),
                CallbackQueryHandler(ma_eta_cancel, pattern=r"^eta:cancel$"),
            ],
            MA_ETA_TIME: [
                CallbackQueryHandler(ma_eta_time, pattern=r"^eta:time:\d{4}$"),
                CallbackQueryHandler(ma_eta_cancel, pattern=r"^eta:cancel$"),
            ],
            MA_REASON: [
...
# =========================================================================== #
# ETA reminders (24h-before nudge + overdue alerts)
# =========================================================================== #
async def _safe_dm(bot, chat_id: int, text: str, reply_markup=None) -> bool:
    try:
        ...
        log.warning("Reminder DM to %s failed: %s", chat_id, exc)
        return False


async def _run_reminder_check(bot) -> int:
    """Send 24h-before nudges and overdue alerts. Returns how many were sent."""
    now = datetime.now(timezone.utc)
    sent = 0
    for req in list_requests_for_reminders():
        raw = req["eta_at"]
        if not raw:
            continue
        try:
            eta_at = datetime.fromisoformat(raw)
        except ValueError:
            continue
        ...
        if not req["reminded_24h"] and (eta_at - timedelta(hours=24)) <= now < eta_at:
            await _safe_dm(
                bot, req["model_id"],
                f"⏰ Reminder: request #{req['id']} ({type_label}) is due in about a day "
                f"(ETA {req['eta']}). Tap below when it's ready.",
                deliver_content(req["id"]))
            update_request(req["id"], reminded_24h=1)
            sent += 1

        if not req["reminded_overdue"] and now >= eta_at:
            await _safe_dm(
                bot, req["model_id"],
                f"⚠️ Request #{req['id']} ({type_label}) is overdue — your ETA was {req['eta']}. "
                "Please deliver as soon as you can.",
                deliver_content(req["id"]))
            alert = (f"⚠️ Overdue: request #{req['id']} ({type_label}) for "
                     f"{_name(req['model_id'])} — ETA was {req['eta']}, not delivered yet.")
            audience = {req["requester_id"]}
            for owner_user in list_by_role(Role.OWNER.value):
                audience.add(owner_user["telegram_id"])
            audience.discard(req["model_id"])
            for chat_id in audience:
                await _safe_dm(bot, chat_id, alert)
            update_request(req["id"], reminded_overdue=1)
            sent += 1
    return sent
...
async def _reminder_loop(app: Application) -> None:
    ...
    while True:
        try:
            count = await _run_reminder_check(app.bot)
            if count:
                log.info("Reminder check sent %d reminder(s)", count)
        except Exception as exc:  # noqa: BLE001
            log.warning("Reminder check failed: %s", exc)
...
async def remindnow_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if role_of(update.effective_user.id) != Role.OWNER.value:
        await send(update, context, "That command is for the agency owner only.")
        return
    count = await _run_reminder_check(context.bot)
