...
def add_reel(plan_id: int, idx: int, **fields) -> int:
    with _conn() as c:
        cur = c.execute(
            "INSERT INTO reels (plan_id, idx, day_label, concept, inspiration, hook, caption, "
            "shoot, audio, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'todo', ?)",
            (plan_id, idx, fields.get("day_label"), fields.get("concept"),
             fields.get("inspiration"), fields.get("hook"), fields.get("caption"),
             fields.get("shoot"), fields.get("audio"), _now()))
        return int(cur.lastrowid)
...
def owner_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✨ Generate reel plan", callback_data="menu:generate")],
...
def model_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("🎬 This week's reels", callback_data="menu:myreels")]])
...
def approve_keyboard(plan_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Approve & send", callback_data=f"plan:approve:{plan_id}")],
        [InlineKeyboardButton("🔄 Regenerate", callback_data=f"plan:regen:{plan_id}"),
         InlineKeyboardButton("🗑 Discard", callback_data=f"plan:discard:{plan_id}")],
    ])
...
async def _register_new_user(update, context, name, config) -> None:
    ...
    if (config.owner_id and tg_user.id == config.owner_id) or count_users() == 0:
        ...
        await send(
            ...
            "generate her weekly reel plan — by hand or automatically every week.",
...
async def _show_home(update, context, role: str, name: str, prefix: str = "") -> None:
    if role == Role.OWNER.value:
        ...
        body = (f"Hi {name}! Each week you'll get a fresh set of reels to shoot, with "
                "captions and instructions. Tap below to see this week's.")
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if role == Role.OWNER.value:
        lines = ["You're the Owner.", "",
                 "/generate — make a reel plan for a model",
        ...
        lines = ["You're a Model.", "",
                 "/myreels — this week's reels to shoot",
                 "/help — this help"]
...
async def scrape_handles(handles: Sequence[str], results_per: int = 6) -> list[dict]:
    """Best-effort: fetch recent posts for the given IG handles via Apify.

    Returns a list of compact post dicts. Never raises — on any problem it
    returns whatever it has (often an empty list), so the pipeline keeps going.
    """
    ...
    endpoint = (f"{APIFY_API}/acts/{CONFIG.apify_actor}/run-sync-get-dataset-items"
    ...
    try:
        ...
        log.warning("Apify scrape failed: %s", exc)
        return []
    ...
    for it in items:
        ...
        posts.append({
            ...
            "type": it.get("type") or it.get("productType") or "",
        })
    return posts


def summarize_posts(posts: list[dict], limit: int = 30) -> str:
    """Compact, token-light summary of scraped posts for the prompt."""
    if not posts:
        return ""
    def score(p):
        return (p.get("views") or 0) + (p.get("likes") or 0) * 3
    top = sorted(posts, key=score, reverse=True)[:limit]
    lines = []
    for p in top:
        metric = f"{p.get('views') or 0} views / {p.get('likes') or 0} likes"
        cap = (p.get("caption") or "").replace("\n", " ").strip()
        lines.append(f"- @{p.get('owner','')} ({metric}) {p.get('url','')}\n  {cap[:200]}")
    return "\n".join(lines)


# =========================================================================== #
# Claude generation (structured 14-reel plan)
# =========================================================================== #
_GEN_SYSTEM = (
    "You are a senior short-form content strategist for an Instagram talent agency. "
    "You design weekly Instagram Reels plans that grow a creator's audience and drive "
    "profile visits and DMs. Your plans are specific and immediately shootable: every "
    "reel has a concrete concept, a strong on-screen hook, a ready-to-post caption with "
    "relevant hashtags, clear shoot instructions, and an audio idea. You study what is "
    "working for similar creators and adapt it to the creator's own niche and tone — you "
    "never tell her to post something off-brand or that crosses the boundaries implied "
    "by her described tone. Keep captions in her voice. Spread reels across the week."
)


def _build_gen_prompt(program: sqlite3.Row, model_name: str, scraped: str, reels_n: int) -> str:
    comps = program_competitors(program)
    parts = [
        f"Create a {reels_n}-reel Instagram plan for the coming week for {model_name}.",
        "",
        f"Her Instagram: @{program['ig_handle']}" if program["ig_handle"] else "",
        f"Niche: {program['niche'] or 'general lifestyle'}",
        f"Tone / style to write in: {program['tone'] or 'authentic and friendly'}",
        f"Creators she is benchmarked against: {', '.join('@'+c for c in comps) if comps else '(none provided)'}",
        "",
    ]
    if scraped:
        parts += [
            "Here is recent top-performing content from those creators and her own account. "
            "Use it as inspiration — spot the formats, hooks and topics that are working and "
            "adapt them to her niche and tone. Reference the most relevant link in each reel's "
            "'inspiration' field when one fits:",
            "",
            scraped,
            "",
        ]
        ...
        parts += [
            "No live scraped data is available this week. Use your knowledge of what performs "
            "in her niche and for creators like the ones listed. For each reel's 'inspiration', "
            "describe the reference format or example to model it on.",
            "",
        ]
    parts += [
        f"Produce exactly {reels_n} reels and a short overall theme for the week.",
        "Keep EVERY field tight and scannable — no rambling:",
        "- day_label: spread evenly across the week as 'Mon AM', 'Mon PM', 'Tue AM', … "
        f"(for {reels_n} reels, ~2 per day Mon–Sun).",
        "- concept: ONE plain sentence — the idea.",
        "- hook: the EXACT on-screen text for the first 2 seconds (a few scroll-stopping words).",
        "- shoot: 1–2 short sentences on how to film it on a phone.",
        "- audio: one short line (a trending-sound type or a specific track idea).",
        "- caption: a real, postable caption in her voice (1–3 short lines) then 4–6 hashtags.",
        "- inspiration: if example posts are provided above, put the single most relevant reel "
        "URL here; otherwise name ONE creator + the exact format to copy "
        "(e.g. '@creator — 3-clip outfit reveal').",
        "Every reel must be concrete and shootable today. Stay strictly within her tone and "
        "niche — never go off-brand.",
    ]
    return "\n".join(p for p in parts if p is not None)


def _generate_plan_sync(program_data: dict, model_name: str, scraped: str, reels_n: int) -> dict:
    """Blocking Anthropic call. Run via asyncio.to_thread. Returns a plain dict."""
    import anthropic
    ...
    # Rebuild a lightweight object exposing the few fields the prompt needs.
    ...
    prompt = _build_gen_prompt(_P(prog_obj), model_name, scraped, reels_n)
    ...
    return {
        "theme": plan.theme,
        "reels": [r.model_dump() for r in plan.reels],
    }


async def generate_plan(model_id: int, created_by: int) -> Optional[int]:
    """Scrape (optional) -> Claude -> persist a draft plan. Returns plan_id or None."""
    program = get_program(model_id)
    model = get_user(model_id)
    if program is None or model is None:
        return None
    reels_n = program["reels_per_week"] or (CONFIG.reels_per_week if CONFIG else 14)

    handles = []
    if program["ig_handle"]:
        handles.append(program["ig_handle"])
    handles += program_competitors(program)
    posts = await scrape_handles(handles)
    scraped = summarize_posts(posts)
    ...
    result = await asyncio.to_thread(
        _generate_plan_sync, program_data, model["display_name"], scraped, reels_n)

    week = _week_start(datetime.now(_tz()).date())
    plan_id = create_plan(model_id, week, result.get("theme", ""), created_by)
    for i, reel in enumerate(result.get("reels", []), start=1):
        add_reel(
            plan_id, i,
            day_label=reel.get("day_label"),
            concept=reel.get("concept"),
            inspiration=reel.get("inspiration"),
            hook=reel.get("hook"),
            caption=reel.get("caption"),
            shoot=reel.get("shoot"),
            audio=reel.get("audio"),
        )
    return plan_id
...
async def _generate_pick(update, context) -> None:
    ...
    if not models:
        await send(update, context,
                   ...
                   reply_markup=owner_menu())
        return
    await send(update, context, "Generate a reel plan for which model?",
...
async def _run_generation(update, context, model_id: int, q=None) -> None:
    model = get_user(model_id)
    name = model["display_name"] if model else f"#{model_id}"
    note = "" if apify_enabled() else " (no live scraping — using niche knowledge)"
    msg_text = f"✨ Generating {name}'s reel plan{note}…\nThis takes ~30–60s."
    if q:
        try:
    ...
    if plan_id is None:
        await send(update, context, "Couldn't generate — is the program set up?",
                   reply_markup=owner_menu())
        return
    await _send_plan_for_review(context.bot, update.effective_chat.id, plan_id)


def plan_overview_text(plan_id: int) -> str:
    plan = get_plan(plan_id)
    reels = list_reels(plan_id)
    model = get_user(plan["model_id"]) if plan else None
    name = model["display_name"] if model else "model"
    head = [f"🗓 {name} — week of {plan['week_start']}",
            f"Theme: {plan['theme'] or '—'}",
            f"{len(reels)} reels", ""]
    for r in reels:
        head.append(f"{r['idx']}. [{r['day_label'] or '—'}] {r['concept']}")
    head.append("")
    head.append("Review the full reels below, then Approve to send to her, or Regenerate.")
    return "\n".join(head)


def reel_detail_text(r: sqlite3.Row, model_name: Optional[str] = None) -> str:
    """Clean, scannable card. Telegram HTML — send with parse_mode='HTML'."""
    def e(s):
        return html.escape(str(s)) if s else ""
    title = f"🎬 <b>Reel {r['idx']}"
    if r["day_label"]:
        title += f" · {e(r['day_label'])}"
    title += "</b>"
    if model_name:
        title += f" — {e(model_name)}"
    lines = [title, ""]
    lines.append(f"💡 <b>Idea:</b> {e(r['concept'])}")
    if r["hook"]:
        lines.append(f"🪝 <b>Hook:</b> {e(r['hook'])}")
    if r["shoot"]:
        lines.append(f"🎥 <b>Film:</b> {e(r['shoot'])}")
    if r["audio"]:
        lines.append(f"🎵 <b>Audio:</b> {e(r['audio'])}")
    if r["inspiration"]:
        lines.append(f"🔎 <b>Inspo:</b> {e(r['inspiration'])}")
    if r["caption"]:
        lines.append("")
        lines.append("📝 <b>Caption</b>")
        lines.append(e(r["caption"]))
    return "\n".join(lines)


async def _send_plan_for_review(bot, chat_id: int, plan_id: int) -> None:
    await bot.send_message(chat_id=chat_id, text=plan_overview_text(plan_id),
                           disable_web_page_preview=True)
    for r in list_reels(plan_id):
        await bot.send_message(chat_id=chat_id, text=reel_detail_text(r),
                               parse_mode="HTML", disable_web_page_preview=True)
    await bot.send_message(
        chat_id=chat_id,
        text="👆 That's the full plan. Approve to send it to her, or regenerate.",
        reply_markup=approve_keyboard(plan_id))
...
async def plan_action_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "discard":
        ...
        await q.edit_message_text("🗑 Plan discarded.")
        return
    ...
    if action == "approve":
        await q.answer()
        await _approve_plan(context, plan_id, q)


async def _approve_plan(context, plan_id: int, q=None) -> None:
    plan = get_plan(plan_id)
    if plan is None:
        return
    ...
    if notion_enabled():
        try:
            ...
            if page_id:
                update_plan(plan_id, notion_page_id=page_id)
        except Exception as exc:  # noqa: BLE001
            log.warning("Notion page failed for plan %s: %s", plan_id, exc)

    # Deliver to the model
    delivered = await deliver_plan_to_model(context, plan_id)

    summary = f"✅ Approved and sent to {model['display_name'] if model else 'model'}."
    if delivered is False:
        summary = (f"✅ Approved, but I couldn't message "
                   f"{model['display_name'] if model else 'her'} — she may not have started "
                   "the bot yet. Share her invite link.")
    if page_url:
        summary += f"\n📄 Notion: {page_url}"
    if q:
        try:
            ...
            await context.bot.send_message(chat_id=CONFIG.owner_id, text=summary)
    else:
        for o in list_by_role(Role.OWNER.value):
            await _safe_dm(context.bot, o["telegram_id"], summary)


async def deliver_plan_to_model(context, plan_id: int) -> bool:
    plan = get_plan(plan_id)
    if plan is None:
        return False
    model_id = plan["model_id"]
    reels = list_reels(plan_id)
    intro = (f"🎬 Your reel plan for the week of {plan['week_start']} is ready!\n"
             f"Theme: {plan['theme'] or '—'}\n{len(reels)} reels — tap each as you shoot it. "
             "Let's go! 🔥")
    try:
        await context.bot.send_message(chat_id=model_id, text=intro)
    except Forbidden:
        return False
    except TelegramError as exc:
        log.warning("Could not message model %s: %s", model_id, exc)
        return False
    for r in reels:
        try:
            await context.bot.send_message(
                chat_id=model_id, text=reel_detail_text(r),
                reply_markup=reel_card_kb(r["id"], r["status"]),
                parse_mode="HTML", disable_web_page_preview=True)
        except TelegramError as exc:
            log.warning("Could not send reel %s to model: %s", r["id"], exc)
    return True


# =========================================================================== #
# Model: this week's reels + tracking
# =========================================================================== #
...
async def _show_my_reels(update, context) -> None:
    ...
    if role_of(uid) != Role.MODEL.value:
        await send(update, context, "Only models have a reel plan.")
        return
    plan = latest_plan_for_model(uid, status=PlanStatus.APPROVED.value)
    if plan is None:
        await send(update, context,
                   "No reel plan yet — you'll get one as soon as it's approved. 💛")
        return
    reels = list_reels(plan["id"])
    done = sum(1 for r in reels if r["status"] == ReelStatus.SHOT.value)
    await send(update, context,
               f"🎬 Week of {plan['week_start']} — {done}/{len(reels)} shot\n"
               f"Theme: {plan['theme'] or '—'}")
    for r in reels:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=reel_detail_text(r),
            reply_markup=reel_card_kb(r["id"], r["status"]),
            parse_mode="HTML", disable_web_page_preview=True)
...
def _plan_blocks(plan_id: int) -> list[dict]:
    plan = get_plan(plan_id)
    reels = list_reels(plan_id)
    blocks = [_para(f"Theme: {plan['theme'] or '—'} · {len(reels)} reels")]
    for r in reels:
        blocks.append(_heading(f"{r['idx']}. [{r['day_label'] or '—'}] {r['concept']}", 3))
        if r["hook"]:
            blocks.append(_para(f"🪝 Hook: {r['hook']}"))
        if r["shoot"]:
            blocks.append(_para(f"📹 Shoot: {r['shoot']}"))
        if r["audio"]:
            blocks.append(_para(f"🎵 Audio: {r['audio']}"))
        if r["inspiration"]:
            blocks.append(_para(f"🔗 Inspiration: {r['inspiration']}"))
        if r["caption"]:
            blocks.append(_para(f"📝 Caption: {r['caption']}"))
...
async def notion_create_plan_page(plan_id: int):
    ...
    title = f"{model['display_name'] if model else 'Model'} — week of {plan['week_start']}"
    payload = {
        "parent": {"type": "page_id", "page_id": _notion_id(CONFIG.notion_parent_page_id)},
        "properties": {"title": {"title": [{"type": "text", "text": {"content": title}}]}},
        "children": _plan_blocks(plan_id),
    }
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post(f"{NOTION_API}/pages", headers=_notion_headers(), json=payload)
        r.raise_for_status()
        data = r.json()
...
async def _run_weekly_check(app: Application) -> int:
    ...
    for program in list_active_programs():
        ...
        if plan_id:
            ...
            for o in owners:
                await _safe_dm(app.bot, o["telegram_id"],
                               f"🗓 New draft reel plan ready for {model['display_name']} — "
                               f"review it below.")
                try:
                    await _send_plan_for_review(app.bot, o["telegram_id"], plan_id)
                except Exception as exc:  # noqa: BLE001
                    log.warning("Could not push draft to owner %s: %s", o["telegram_id"], exc)
    return generated
...
async def gennow_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for program in list_active_programs():
        ...
        if plan_id:
            n += 1
            await _send_plan_for_review(context.bot, update.effective_chat.id, plan_id)
    await send(update, context, f"Done — {n} draft plan(s) generated.",
               reply_markup=owner_menu())
...
async def fallback_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if role == Role.OWNER.value:
        ...
        await send(update, context, "Tap below for this week's reels.", reply_markup=model_menu())
    else:
        await send(update, context, "You're not registered. Ask the agency owner for an invite link.")
...
def build_application(config: Config) -> Application:
    ...
    app.add_handler(CallbackQueryHandler(plan_action_cb, pattern=r"^plan:(approve|regen|discard):\d+$"))
