...
def init_db() -> None:
    with _conn() as c:
        ...
        for col in ("notion_url", "account_reviews"):
            if col not in pcols:
...
def _parse_ig_items(items) -> list[dict]:
    ...
    for it in items:
        ...
        posts.append({
            ...
            "timestamp": it.get("timestamp") or it.get("takenAt") or "",
            "music": music,
        })
    return posts
...
def _age_days(p: dict) -> Optional[int]:
    ts = p.get("timestamp")
    if not ts:
        return None
    try:
...
def _trend_key(p: dict):
    """Prefer what's trending NOW (recent) over all-time top-viewed."""
    age = _age_days(p)
    recent = 1 if (age is not None and age <= 60) else 0
    return (recent, _ref_score(p))
...
def rank_references(posts: list[dict], limit: int = 30, per_owner: int = 5) -> list[dict]:
    ...
    order: list[str] = []
    for p in sorted(reels, key=_trend_key, reverse=True):
...
def references_text(refs: list[dict], own_handle: Optional[str] = None) -> str:
    """Numbered list of real references for the prompt (1-based)."""
    own = (own_handle or "").lower().lstrip("@")
    lines = []
    for i, p in enumerate(refs, start=1):
        ...
        mine = "  [HER OWN PAST CONTENT]" if own and (p.get("owner") or "").lower() == own else ""
        lines.append(f"{i}. @{p.get('owner','')} — {p.get('views') or 0} views{when}{mine}"
                     f"{music}\n   {p.get('url','')}\n   {cap}")
    return "\n".join(lines)
...
# =========================================================================== #
_GEN_SYSTEM = (
    "You are an elite short-form strategist for a creator agency. The GOAL is to build the "
    "creator's BRAND and funnel followers toward her OnlyFans — NOT to chase raw views. You "
    "operate at the highest standard: a low-effort, off-persona, or view-chasing suggestion is "
    "a FAILURE. Every idea must pass these hard filters before it makes the plan:\n"
    "1. PERSONA FIT (hard filter): it must clearly fit THIS creator's persona. A reel that went "
    "viral but isn't her — e.g. a fart-jar, animal or prank stunt for a mommy/kitchen MILF — "
    "FAILS, regardless of view count. If it doesn't obviously fit her, leave it out.\n"
    "2. TRENDING, NOT TOP-VIEWED: use what is genuinely trending NOW for her niche (recent posts, "
    "trending audio/formats) — not all-time top-viewed videos, which are often viral because "
    "they're ridiculous. High views != on-brand.\n"
    "3. REPLICABLE: she must be able to realistically shoot it with what she has; if not, drop it.\n"
    "4. FORMAT, NOT TOPIC: treat each reference reel as a format template (hook/structure/pacing/"
    "audio) and apply it to her world; never copy an off-persona topic.\n"
    "5. BRAND + FUNNEL: ideas and captions grow her brand and pull viewers toward her OF/DMs, "
    "tastefully (not explicit).\n"
    "The Caption is her own casual, lowercase, flirty voice, paste-ready. Brief fields are "
    "concise and directive. Cite the exact reference reel by its number. Quality and brand fit "
    "over volume. Every idea in a plan must be DISTINCT — never repeat a concept, format or hook; "
    "a duplicate or near-duplicate idea is a failure."
)
...
def _build_gen_prompt(program: sqlite3.Row, model_name: str, refs_text: str,
    ...
    comps = program_competitors(program)
    niche = program["niche"] or "general lifestyle"
    parts = [
        f"Build {model_name}'s weekly content plan. GOAL: grow her BRAND and funnel followers to "
        "her OnlyFans — not chase raw views.",
        f"Deliver {n_sched} SCHEDULED videos (2 per day across the week) + {n_alt} SWAP options.",
        "",
        "HER PERSONA PROFILE — use as a HARD FILTER; every idea must fit this:",
        f"- Instagram: @{program['ig_handle']}" if program["ig_handle"] else "",
        f"- Niche / persona: {niche}",
        f"- Voice for captions: {program['tone'] or 'casual and friendly'}",
        "",
        f"Tagged reference accounts to EVALUATE: "
        f"{', '.join('@'+c for c in comps) if comps else '(none)'}",
        "",
    ]
    if refs_text:
        parts += [
            "REAL reels trending now (numbered; from the tagged accounts, her niche hashtags, "
            "and her own account). Items marked [HER OWN PAST CONTENT] are her past posts.",
            "Treat each as a FORMAT TEMPLATE (hook/structure/pacing/audio) to apply to her "
            "persona — cite the one you used in 'reference_index'.",
            "FILTERS — reject anything that fails: (a) must FIT her persona — off-persona viral "
            "junk (fart/animal/prank reels) is rejected no matter the views; (b) must be "
            "TRENDING NOW (prefer recent + trending audio, not all-time top-viewed); (c) must be "
            "REALISTICALLY recreatable by her. Adapt the FORMAT, never the literal off-persona "
            "topic (a 'guess the weird thing' trend → 'guess what I'm cooking').",
            "",
            refs_text,
            "",
        ]
        ...
        parts += [
            "No live references came back. Set reference_index to -1, but still produce specific, "
            "currently-trending ideas that fit her persona and are realistically recreatable.",
            "",
        ]
    parts += [
        "Return THREE things:",
        "1. account_reviews: one entry PER tagged account above — on_brand (true/false) and a "
        "one-line note on whether it fits her brand/persona and its funnel potential to OF, with "
        "a recommendation (keep / swap out / pull more like it). Judge on brand alignment, NOT "
        "follower count or virality.",
        f"2. scheduled: EXACTLY {n_sched} ideas; day_label spread 2/day ('Mon AM','Mon PM',…).",
        f"3. alternates: EXACTLY {n_alt} ideas; day_label 'Swap'.",
        "Each idea has:",
        "- reference_index: the reel it adapts (or -1).",
        "- concept: one line — the SPECIFIC, on-persona, realistically-shootable video.",
        "- why: one line tied to HER — her strengths and her PAST WINS. When you adapt a format "
        "from one of her [HER OWN PAST CONTENT] reels, say so (e.g. 'your [topic] reel used this "
        "and performed well'). Never a generic reason that fits any creator.",
        "- audio: the trending sound by name (use the reference's audio if given).",
        "- hook: the exact on-screen text — punchy, in the reference's style.",
        "- caption: paste-ready, her casual lowercase flirty voice; nudge toward her OF/DMs "
        "tastefully.",
        "- hashtags: one line of 4–8 relevant hashtags.",
        f"STANDARD: highest quality only. Off-persona, view-chasing, un-shootable or generic "
        f"ideas are FAILURES. Every idea must obviously belong on HER account ({niche}). "
        "Quality and brand fit over volume.",
        f"VARIETY IS MANDATORY: all {n_sched + n_alt} ideas must be DISTINCT — each a different "
        "concept AND format. Never repeat the same idea, format, hook pattern, or reference "
        "treatment twice. Spread across as many different references as possible. If you run low "
        "on references, create fresh on-persona angles rather than repeating — a repeated or "
        "near-duplicate idea is a FAILURE.",
    ]
...
def _generate_plan_sync(program_data: dict, model_name: str, refs_text: str,
    ...
    class ContentPlan(BaseModel):
        account_reviews: list[AccountReview]
        scheduled: list[Idea]
    ...
    return {
        "account_reviews": [a.model_dump() for a in plan.account_reviews],
...
async def generate_plan(model_id: int, created_by: int) -> Optional[int]:
    ...
    # Pool = competitor/own reels (per-account) + trending reels under her niche hashtags.
    # The hashtag trends keep references on-niche even when a competitor is weak/off-topic.
    pool = await _scrape_each(handles)
    pool += await scrape_hashtags(_niche_hashtags(program["niche"]))
    refs = rank_references(pool)
    refs_text = references_text(refs, program["ig_handle"])   # tags her own past content
    ...
    if result.get("account_reviews"):
        update_plan(plan_id, account_reviews=json.dumps(result["account_reviews"]))
    idx = 1
...
async def scrape_hashtags(tags: Sequence[str], results_per: int = 60) -> list[dict]:
    ...
    return _parse_ig_items(await _apify_run(CONFIG.apify_hashtag_actor, payload))
...
async def _finish_plan(bot, chat_id: int, plan_id: int) -> Optional[str]:
    ...
    flagged = ""
    try:
        reviews = json.loads(plan["account_reviews"]) if plan["account_reviews"] else []
        off = [r["handle"] for r in reviews if not r.get("on_brand")]
        if off:
            flagged = "\n⚠️ Off-brand accounts to reconsider: " + ", ".join("@" + h for h in off)
...
def _review_blocks(plan_id: int) -> list[dict]:
    """Top section: verdict on each tagged reference account."""
    plan = get_plan(plan_id)
    raw = plan["account_reviews"] if (plan and "account_reviews" in plan.keys()) else None
    if not raw:
        return []
    try:
    ...
    if not reviews:
        return []
    blocks = [_heading("🔎 Reference account review", 3)]
    for rv in reviews:
        mark = "✅" if rv.get("on_brand") else "⚠️"
        blocks.append(_field(f"{mark} @{rv.get('handle','')}", rv.get("note") or "—"))
    blocks.append(_heading("📅 This week's content", 3))
    return blocks
...
def _plan_blocks(plan_id: int, image_mode: str = "column") -> list[dict]:
    ...
    blocks: list[dict] = _review_blocks(plan_id)
    for r in week:
