...
def _build_gen_prompt(program: sqlite3.Row, model_name: str, scraped: str, reels_n: int) -> str:
    ...
    parts += [
        f"Produce exactly {reels_n} reels. Give the week a short theme. Spread the reels "
        "across the days (roughly 2 per day if 14). Each reel must be concrete and shootable "
        "today with a phone. Keep captions in her voice and within her stated tone.",
    ]
...
def reel_detail_text(r: sqlite3.Row, model_name: Optional[str] = None) -> str:
    lines = []
    if model_name:
        lines.append(f"🎬 Reel {r['idx']} — {model_name}")
    else:
        lines.append(f"🎬 Reel {r['idx']}")
    if r["day_label"]:
        lines.append(f"📅 {r['day_label']}")
    lines.append("")
    lines.append(f"💡 Concept: {r['concept']}")
    if r["hook"]:
        lines.append(f"🪝 Hook: {r['hook']}")
    if r["shoot"]:
        lines.append(f"📹 Shoot: {r['shoot']}")
    if r["audio"]:
        lines.append(f"🎵 Audio: {r['audio']}")
    if r["inspiration"]:
        lines.append(f"🔗 Inspiration: {r['inspiration']}")
    if r["caption"]:
        lines.append("")
