...
def rank_references(posts: list[dict], limit: int = 20) -> list[dict]:
    """Real reference REELS only (photos excluded), ranked by engagement.

    If nothing qualifies as a reel, returns [] so the plan honestly says
    'no live reference found' instead of pointing at a photo.
    """
    reels = [p for p in posts if p.get("url") and _is_reel(p)]
    return sorted(reels, key=_ref_score, reverse=True)[:limit]
...
async def generate_plan(model_id: int, created_by: int) -> Optional[int]:
    ...
    # Prefer the dedicated reel actor; fall back to the post scraper (reels filtered).
    refs = rank_references(await scrape_reels(handles))
    if not refs:
        refs = rank_references(await scrape_handles(handles))
