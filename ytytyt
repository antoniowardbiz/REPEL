...
def load_config() -> Config:
    ...
    alternates = _int_env("ALTERNATES", 6)
    alternates = alternates if 0 <= alternates <= 20 else 6
...
_GEN_SYSTEM = (
    ...
    "paste-ready. Always cite the exact reference reel by its number."
)
...
def _build_gen_prompt(program: sqlite3.Row, model_name: str, refs_text: str,
    ...
    if refs_text:
        parts += [
            "REAL reels performing in her niche RIGHT NOW (each numbered). Base every idea on "
            "ONE of them, cite it in 'reference_index', and REMAKE its format for her — copy "
            "the hook style, structure and pacing, and use its audio. Use ONLY these numbers; "
